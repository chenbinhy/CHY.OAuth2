...
namespace OAuthAuthorizationServer.Code {
	...
	internal class OAuth2AuthorizationServer : IAuthorizationServerHost {
        ...
        public OAuth2AuthorizationServer(DatabaseKeyNonceStore keyNonceStore, IRepository<Client> clientRep)
        {
            m_KeyNonceStore = keyNonceStore;
		...
		public IClientDescription GetClient(string clientIdentifier) {
           var clientInfo = m_ClientRep.Get(s => s.ClientIdentifier == clientIdentifier);
           if (clientInfo == null)
           {
               throw new ArgumentOutOfRangeException("clientIdentifier");
           }
           ClientExt clientExt = new ClientExt(clientInfo);
		...
		public bool CanBeAutoApproved(EndUserAuthorizationRequest authorizationRequest) {
			...
			if (authorizationRequest.ResponseType == EndUserAuthorizationResponseType.AuthorizationCode) {
                ...
                var requestingClient = m_ClientRep.Get(s => s.ClientIdentifier == authorizationRequest.ClientIdentifier);
